    ...
    def __init__(self,
        ...
        if help_cmd:
            ...
            def help() -> None:
                ...
                for i in self.cmd:
                    if i["alias"] != []:
                        text += "Alias    "+ ", ".join(i["alias"])+" -> "+i["name"]+" "+" ".join(map(str, i["args"]))+"\n"
                    else:
                        text += "- "+i["name"]+" "+" ".join(map(str, i["args"]))+"\n"
                    if i["description"] is not None:
                        text += i["description"]+"\n"
                echo(text[:-1], animation=self.animation, cooldown=self.cooldown, logs=self.logs)

        @self.command(alias=["leave"])
        def exit() -> None:
            """Close the terminal."""
            os.kill(os.getpid(), 9)
                    ...
                    if i["name"] == entry.split(" ")[0] or entry.split(" ")[0] in i["alias"]:
                        ...
                        for nmb, arg in enumerate(entry.split(" ")[1:len(i["args"])+1]):
                            if py_version[0] == "3" and py_version[1] >= "10":
                                ...
                                if i["types"][nmb] == "<class 'int'>": args.append(int(arg))
                                elif i["types"][nmb] == "<class 'bytes'>": args.append(arg.encode())
                                elif i["types"][nmb] == "<class 'float'>": args.append(float(arg))
                                elif i["types"][nmb] == "<class 'bool'>": args.append(bool(arg))
