        ...
        def decorator(func: callable) -> callable:
            def wrapper(name:str, 
                ...
                for arg in inspect.signature(func).parameters.items():
                    types.append(str(arg[1].annotation).removeprefix("<class '").removesuffix("'>"))
                    args.append(f"[{arg[0]}]")
                exist = False
                    ...
                    if i["name"] == entry.split(" ")[0] or entry.split(" ")[0] in i["alias"]:
                        ...
                        for nmb, arg in enumerate(entry.split(" ")[1:len(i["args"])+1]):
                            for type in i["types"][nmb].split(" | "):
                                try:
                                    match type:
                                        case "int": args.append(int(arg))
                                        case "float": args.append(float(arg))
                                        case "complex": args.append(complex(arg))
                                        case "bool": args.append(bool(arg))
                                        case "bytes": args.append(bytes(arg))
                                        case "bytearray": args.append(bytearray(arg))
